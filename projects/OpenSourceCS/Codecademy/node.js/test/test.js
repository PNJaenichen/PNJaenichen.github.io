...
var { dieRoller, pool_adjuster, missile_attack } = require('..\\OWS_Adjudicator_v18');
...
describe('Dice Roller', () => {
  ...
  const testArray = (dice) => {
    const resultArray = [];
    for (let i = 0; i < 5; i++) {
      resultArray.push(dieRoller(dice));
    }
    return resultArray;
  };
...
describe('Dice Promotion', () => {
  describe('of all dice', () => {
    it('that are the same and not a d20', () => {
      const startPool = {'4': 3};
      const promoPool = pool_adjuster.promoteAll(startPool);
  ...
  describe('of one die', () => {
    it('and promotes a single die pool properly', () => {
      const startPool = {'4': 1};
      const promoPool = pool_adjuster.promoteOne(startPool);
  ...
  describe('combines all dice', () => {
    it('into one of the next highest die', () => {
      ...
      const promoPool = pool_adjuster.allForOne(startPool);
    ...
    it('but never returns more than a d16 for ASW', () => {
      ...
      const promoPool = pool_adjuster.allForOne(startPool, true);
...
describe('Dice Demotion', () => {
  describe('of all dice', () => {
    it('that are the same and not a d4', () => {
      const startPool = {'8': 3};
      const promoPool = pool_adjuster.demoteAll(startPool);
      const expectPool = {'6': 3};
      assert.deepStrictEqual(promoPool, expectPool);
    });
    it('and removes d4s', () => {
  ...
  describe('of one die', () => {
    it('and demotes a single die pool properly', () => {
      const startPool = {'8': 1};
      const promoPool = pool_adjuster.demoteOne(startPool);
      const expectPool = {'6': 1};
      assert.deepStrictEqual(promoPool, expectPool);
    });
...
// incoming, singles, promotions, demotions, hyper, ballistic

describe('Missile Strike', () => {
  describe('makes adjustments to dice pool', () => {
    it('returns correctly if no adjustments', () => {
      assert.deepStrictEqual(missile_attack.pool_adjust({'10': 2}, 0, 0, 0), {'10': 2});
    });
    it('returns correctly if only promotions made', () => {
      assert.deepStrictEqual(missile_attack.pool_adjust({'10': 2}, 0, 2, 0), {'16': 2})
    }) 
  });
});