export default function PlayParser(htmlReport) {
  ...
  const playBreakDown = rowBreakDown.map(row => row.map(element => [element[0].innerText, element[1].innerText, element[3].innerText.match(/(\d{1,2}:\d{2})/g), element[6].innerText.replace(spaces, "").match(digits), element[7].innerText.replace(spaces, "").match(digits)]));
